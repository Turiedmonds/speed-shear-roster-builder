...
function doGet(e) {
  try {
    ...
    if (action === 'entry-manager') {
      return entryManagerJsonResponse_(entryManagerPublicSetup_(e.parameter.access || ''));
    }
    if (action === 'competitor-entry') {
      return entryManagerJsonResponse_(entryManagerCompetitorSetup_(e.parameter.entry || ''));
    }
...
function doPost(e) {
  try {
    const payload = entryManagerParseRequest_(e);
...
function entryManagerParseRequest_(e) {
  ...
  try {
    ...
    throw new Error('Request data was not valid JSON.');
  }
}
