(() => {
  ...
  function downloadJson(payload,name){const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);}
  ...
  els.gradesContainer.addEventListener('click',async e=>{
    ...
    const card=btn.closest('.grade-card'); const g=gradeById(card?.dataset.gradeId); if(!g)return;
    const action=btn.dataset.action;
    if(action==='add-competitor'){
      const n=card.querySelector('[data-role="quick-name"]'),t=card.querySelector('[data-role="quick-town"]');
      if(!addCompetitor(g.id,n.value,t.value))return setStatus('Enter a name, or check that the competitor is not already listed.','warn'); render();scheduleSave();
    }
    if(action==='bulk-add'){
      const ta=card.querySelector('[data-role="bulk-text"]'); let added=0,skipped=0;
      ta.value.split(/\r?\n/).forEach(line=>{const p=parseLine(line);if(!p)return;if(addCompetitor(g.id,p.name,p.town))added++;else skipped++;});
      card.querySelector('[data-role="bulk-status"]').textContent=`Added: ${added} • Skipped: ${skipped}`; ta.value='';render();scheduleSave();
    }
